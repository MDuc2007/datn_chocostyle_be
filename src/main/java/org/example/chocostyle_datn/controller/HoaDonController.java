...
public class HoaDonController {
    ...
    @GetMapping("/ca-hien-tai/{idNv}")
    public ResponseEntity<?> getHoaDonTrongCa(@PathVariable Integer idNv) {
        try {
            return ResponseEntity.ok(hoaDonService.getHoaDonCaHienTai(idNv));
        } catch (RuntimeException e) {
            return ResponseEntity.badRequest().body(e.getMessage());
        }
    }
