...
import org.example.chocostyle_datn.model.Request.*;
...
public class HoaDonService {
    ...
    public void xacNhanDatHangTaiQuay(Integer idHoaDon, CreateOrderRequest req) {
        ...
        // Map khách hàng
        // Map khách hàng
        if (req.getIdKhachHang() != null && req.getIdKhachHang() > 0) {
            ...
            String ten = req.getTenNguoiNhan() != null ? req.getTenNguoiNhan() : "Khách lẻ";
            String sdt = req.getSdtNguoiNhan() != null ? req.getSdtNguoiNhan() : "";
            String diaChi = req.getDiaChiGiaoHang() != null ? req.getDiaChiGiaoHang() : "";
            ...
            for (CartItemRequest item : req.getSanPhamChiTiet()) {
                ...
                if (sp.getTrangThai() == null || sp.getTrangThai() != 1) {
                    throw new RuntimeException(
                            "Sản phẩm " + sp.getMaChiTietSanPham() + " đã ngừng bán hoặc bị khóa!"
                    );
                }

                if (sp.getIdSanPham() == null || sp.getIdSanPham().getTrangThai() != 1) {
                    throw new RuntimeException(
                            "Sản phẩm " + sp.getMaChiTietSanPham() + " đã bị ẩn hoặc ngừng kinh doanh!"
                    );
                }

                if (sp.getSoLuongTon() < 0) {
                    throw new RuntimeException(
                            "Tồn kho sản phẩm " + sp.getMaChiTietSanPham() + " không hợp lệ!"
                    );
                }

                if (sp.getSoLuongTon() < item.getSoLuong()) {
                    throw new RuntimeException(
                            "Sản phẩm " + sp.getMaChiTietSanPham() + " không đủ số lượng tồn kho!"
                    );
                }
