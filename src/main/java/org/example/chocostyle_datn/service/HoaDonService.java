...
public class HoaDonService {
    ...
    @Autowired private LichSuHoaDonRepository lichSuHoaDonRepository;
    ...
    @Transactional // Annotation này giúp roll-back nếu có lỗi giữa chừng
    public void xoaHoaDonTaiQuay(Integer idHoaDon) {
        // 1. Lấy hóa đơn ra kiểm tra xem có tồn tại không
        HoaDon hoaDon = hoaDonRepository.findById(idHoaDon)
                .orElseThrow(() -> new RuntimeException("Không tìm thấy hóa đơn có ID: " + idHoaDon));

        // Tùy chọn: Chặn không cho xóa hóa đơn đã hoàn thành/giao hàng
        // if (hoaDon.getTrangThai() != 0) {
        //     throw new RuntimeException("Chỉ có thể xóa hóa đơn chưa xác nhận!");
        // }

        // 2. Xóa các dữ liệu liên quan trước (để tránh lỗi khóa ngoại)
        lichSuHoaDonRepository.xoaLichSuTheoIdHoaDon(idHoaDon);
        hoaDonChiTietRepository.xoaChiTietTheoIdHoaDon(idHoaDon);

        // Nếu có bảng thanh toán:
        // thanhToanRepository.xoaThanhToanTheoIdHoaDon(idHoaDon);

        // 3. Cuối cùng mới xóa Hóa đơn chính
